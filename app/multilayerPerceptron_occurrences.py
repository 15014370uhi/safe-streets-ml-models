...
# for each record of crimes in a sector, generate and save model
for record in sectors:
    ...
    classifier = make_pipeline(StandardScaler(), MLPClassifier(solver='adam', max_iter=200, alpha=1e-05, hidden_layer_sizes=(6,5), random_state=1))     
    ...
    dfResult.to_csv('comparison_tables/multilayerperceptron/MLP_comparison_table_' +
        record['sector'] + '.csv', encoding='utf-8', index=False)
    print('Table saved')    
    
    # save model for this crime sector to file   
    filename = 'MultiLayerPerceptron_Occurrences/MultiLayerPerceptron_' + record['sector'] + '.sav'  
    ...
    plt.axis('equal')
    plotFilename = 'comparison_tables/multilayerperceptron/MLP_Occurrences_' + record['sector'] + '.png'
